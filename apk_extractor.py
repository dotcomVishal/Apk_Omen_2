...
class SignatureInfo(TypedDict):
    """
    APK signing certificate metadata extracted via ``keytool -printcert``.

    Fields are empty strings when keytool is unavailable or parsing fails —
    the report is still emitted; signature data is treated as optional enrichment.
    """
    subject_cn: str          # Certificate Common Name  (CN=...)
    subject_o: str           # Organisation             (O=...)
    subject_ou: str          # Organisational Unit      (OU=...)
    sha256_fingerprint: str  # e.g. "AB:CD:EF:..."
    valid_from: str          # "Mon Jan 01 00:00:00 UTC 2020"
    valid_until: str         # "Sat Dec 31 23:59:59 UTC 2030"
    is_debug_cert: bool      # True when signed with the Android debug key
...
# ──────────────────────────────────────────────────────────────────────────────
# Step 0: APK Signature Extraction
# ──────────────────────────────────────────────────────────────────────────────

# Known subject strings that identify the Android SDK debug signing key.
# Any APK bearing one of these should be treated as a development build.
_DEBUG_CERT_SUBJECTS: frozenset[str] = frozenset(
    {
        "cn=android debug",
        "ou=android",
        "o=android",
...
# keytool field extraction patterns — compiled once at module load.
_KT_SUBJECT_RE:     re.Pattern[str] = re.compile(
    r'(?:Owner|Subject)\s*:\s*(.+)', re.IGNORECASE
)
_KT_SHA256_RE:      re.Pattern[str] = re.compile(
    r'SHA256\s*:\s*([0-9A-Fa-f:]{95})', re.IGNORECASE
)
_KT_VALID_FROM_RE:  re.Pattern[str] = re.compile(
    r'Valid from\s*:\s*(.+?)\s+until', re.IGNORECASE
)
_KT_VALID_UNTIL_RE: re.Pattern[str] = re.compile(
    r'until\s*:\s*(.+)', re.IGNORECASE
)
# Parses individual RDN tokens from a DN string:  "CN=Foo, O=Bar, OU=Baz"
_KT_RDN_RE: re.Pattern[str] = re.compile(
    r'(?:^|,\s*)(?P<key>[A-Za-z]+)=(?P<val>[^,]+)'
)


def _parse_dn(dn_string: str) -> dict[str, str]:
    """
    Extract a flat {key: value} map from an X.500 Distinguished Name string.

    Example input : "CN=John Doe, OU=Engineering, O=Acme Corp, L=NYC, C=US"
    Example output: {"CN": "John Doe", "OU": "Engineering", "O": "Acme Corp", ...}
    """
    return {
        m.group("key").upper(): m.group("val").strip()
        for m in _KT_RDN_RE.finditer(dn_string)
    }


def _empty_signature() -> SignatureInfo:
    """Return a zeroed-out SignatureInfo for error / unavailable cases."""
...
def extract_signature_info(apk_path: Path) -> SignatureInfo:
    """
    Run ``keytool -printcert -jarfile <apk_path>`` and parse the signing
    certificate metadata.

    Failure modes are handled gracefully — if keytool is absent, the APK is
    unsigned, or parsing produces no matches, an empty ``SignatureInfo`` is
    returned so the rest of the pipeline is unaffected.
    ...
    Returns
    -------
    ``SignatureInfo`` TypedDict populated with certificate fields, or an empty
    instance on any failure.
    """
    if shutil.which("keytool") is None:
        logger.warning("keytool not found on PATH — signature extraction skipped.")
        return _empty_signature()

    logger.info("Extracting signing certificate from: %s", apk_path.name)
    ...
    try:
        ...
        logger.warning("keytool timed out for %s", apk_path.name)
        return _empty_signature()
    except OSError as exc:
        logger.warning("keytool execution failed: %s", exc)
        return _empty_signature()

    if result.returncode != 0:
        logger.warning(
            "keytool exited %d for %s: %s",
            result.returncode, apk_path.name, result.stderr.strip()[:200],
        )
        return _empty_signature()

    output = result.stdout

    # ── Parse subject / owner line ────────────────────────────────────────────
    subject_match = _KT_SUBJECT_RE.search(output)
    rdn: dict[str, str] = {}
    subject_raw = ""
    if subject_match:
        subject_raw = subject_match.group(1).strip()
        rdn = _parse_dn(subject_raw)
        logger.debug("Certificate subject: %s", subject_raw)

    # ── Parse SHA-256 fingerprint ─────────────────────────────────────────────
    sha256_match = _KT_SHA256_RE.search(output)
    sha256 = sha256_match.group(1).strip().upper() if sha256_match else ""

    # ── Parse validity window ─────────────────────────────────────────────────
    from_match  = _KT_VALID_FROM_RE.search(output)
    until_match = _KT_VALID_UNTIL_RE.search(output)
    valid_from  = from_match.group(1).strip()  if from_match  else ""
    valid_until = until_match.group(1).strip() if until_match else ""

    # ── Debug certificate detection ───────────────────────────────────────────
    # Any component of the subject matching a known debug token → debug cert.
    subject_lower = subject_raw.lower()
    is_debug = any(token in subject_lower for token in _DEBUG_CERT_SUBJECTS)
    if is_debug:
        logger.warning(
            "APK appears to be signed with a DEBUG certificate: %s", apk_path.name
        )

    sig = SignatureInfo(
        subject_cn=rdn.get("CN", ""),
        subject_o=rdn.get("O", ""),
        subject_ou=rdn.get("OU", ""),
    ...
    logger.info(
        "Signature extracted — CN=%r O=%r debug=%s sha256=%s…",
        sig["subject_cn"], sig["subject_o"], sig["is_debug_cert"],
        sha256[:17] if sha256 else "N/A",
    )
    return sig
...
def run_extraction(apk_path: str | Path) -> ExtractionReport:
    """
    Full synchronous extraction pipeline — Deep Scan + Signature edition.

    Stages
    ──────
    0. Extract signing certificate via keytool  (non-blocking on failure)
    ...
    # ── 0. Signature extraction (runs on the original APK, before decompile) ──
    # Intentionally outside the TemporaryDirectory context — we need the
    # original APK path, not the decompiled output dir.
    signature_info = extract_signature_info(apk_path)
