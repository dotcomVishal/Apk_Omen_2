...
from ai_agent import ask_security_assistant
...
class ChatRequest(BaseModel):
    ...
    question: str = Field(
        ...
        min_length=1,
        max_length=2_000,
        description="The developer's natural-language question about their scan.",
        examples=["Which vulnerabilities are most critical to fix first?"],
    )
    report_context: dict = Field(
        ...,
        description=(
            "The full MappedReport dict returned by the /analyze endpoint. "
            "Injected as grounding context into the AI system prompt."
        ),
    )
...
@app.post(
    ...
    status_code=status.HTTP_200_OK,
    tags=["ai-assistant"],
    summary="Ask the AI security assistant a question about a scan report.",
    response_description='{"answer": "<markdown string>"}',
)
async def chat(body: ChatRequest) -> JSONResponse:
    """
    Submit a natural-language question paired with a scan report and receive
    a grounded AI response from the Groq-hosted security assistant.

    ### How it works
    The full ``report_context`` dict is serialised to JSON and injected into
    the model's system prompt as read-only grounding data.  The model is
    instructed to answer strictly from that data — no hallucinations.

    ### Typical client flow
    1. Call ``POST /analyze`` → receive ``ui_report``.
    2. Store ``ui_report`` in the frontend.
    3. Call ``POST /chat`` with ``{"question": "...", "report_context": ui_report}``.
    4. Stream or display the ``answer`` field.
    ...
    | 503    | Groq service unavailable / timeout |
    | 500    | Unexpected error (misconfigured key, serialisation failure, etc.) |
    """
    logger.info(
        "Chat request received — question_len=%d report_keys=%s",
        len(body.question),
        list(body.report_context.keys()),
    )

    try:
        answer = ask_security_assistant(
            question=body.question,
            report_context=body.report_context,
        )
        logger.info("Chat response ready — answer_len=%d chars", len(answer))
        return JSONResponse(content={"answer": answer}, status_code=status.HTTP_200_OK)

    except ValueError as exc:
        # Blank question slipped past Pydantic (edge case with whitespace-only input)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="AI assistant is not configured. Contact the server administrator.",
        )

    except Exception as exc:
        # Map Groq-specific exceptions to appropriate HTTP status codes
        exc_name = type(exc).__name__
        logger.exception("Chat request failed (%s): %s", exc_name, exc)

        if "RateLimitError" in exc_name:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail="AI assistant rate limit reached. Please wait a moment and try again.",
            )

        if "Timeout" in exc_name or "Connection" in exc_name:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail="AI assistant is temporarily unreachable. Please try again.",
            )

        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"AI assistant encountered an unexpected error: {exc!s}",
        )
...
@app.post(
    ...
    status_code=status.HTTP_200_OK,
    tags=["export"],
    summary="Export a mapped vulnerability report as a professional PDF.",
    response_description="Binary PDF file (application/pdf) as a file download.",
    responses={
        200: {
            "content": {"application/pdf": {}},
            "description": "PDF audit report — triggers browser download.",
        }
    },
)
async def export_pdf(body: dict) -> Response:
    """
    Accept the full ``MappedReport`` JSON body (the exact object returned by
    ``POST /analyze``) and return a multi-page PDF security audit report.
    ...
    const res   = await fetch("/export/pdf", {
        ...
        body:    JSON.stringify(uiReport),   // the /analyze response
    });
    ...
    ### Report structure
    - **Cover page** — package name, scan date, classification band
    - **Table of contents** — auto-generated from vulnerability list
    - **Executive Summary** — severity metrics, pie chart, bar chart, app metadata
    - **Vulnerability Findings** — one section per finding with description,
      remediation, and evidence table (file, line number, match preview)
    - **Permission Inventory** — all declared permissions with danger classification

    ### Error responses
    | Status | Cause |
    |--------|-------|
    | 400    | Empty or malformed request body |
    | 500    | PDF generation failure (ReportLab / matplotlib error) |
    """
    if not body:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Request body must be a non-empty MappedReport JSON object.",
        )

    pkg_name = (
        (body.get("app_profile") or {}).get("package_name", "unknown")
        .replace(" ", "_")
        .replace("/", "_")
    )
    # Sanitise filename: keep alphanumerics, dots, hyphens, underscores
    import re as _re
    safe_pkg = _re.sub(r"[^\w.\-]", "_", pkg_name)[:60]
    filename = f"security_report_{safe_pkg}.pdf"

    logger.info(
        "PDF export requested — package=%s vulnerabilities=%d",
        pkg_name,
        len((body.get("vulnerabilities") or [])),
    )

    try:
        pdf_bytes: bytes = generate_pdf_report(body)

        logger.info(
            "PDF export complete — filename=%s size=%d bytes",
        ...
        logger.exception("PDF generation failed for package '%s': %s", pkg_name, exc)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
